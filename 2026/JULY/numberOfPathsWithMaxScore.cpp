Number of Paths with Max Scoreclass Solution {
public:
...
// Time Complexity: O(n^2) where n is the size of the board. We are iterating through each cell of the board once, and for each cell, we are checking at most three neighboring cells (down, right, and diagonal).
// Space Complexity: O(n^2) where n is the size of the board. We are using two 2D vectors, maxScore and ways, each of size n x n, to store the maximum score and the number of ways to reach each cell.